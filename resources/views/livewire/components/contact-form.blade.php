<div>
    @if($isSubmitted)
        <div class="rounded-2xl bg-teal-50 dark:bg-teal-900/30 p-5 border border-teal-100 dark:border-teal-800 text-center animate-fade-in">
            <div class="mx-auto flex items-center justify-center h-10 w-10 rounded-full bg-teal-100 dark:bg-teal-800 mb-3">
            ...
            <h3 class="text-base font-medium text-teal-800 dark:text-teal-300">{{ $successMessage }}</h3>
            ...
            <button wire:click="$set('isSubmitted', false)" class="mt-4 text-sm font-medium text-teal-600 dark:text-teal-400 hover:text-teal-500 transition-colors">
                Enviar otro mensaje
            </button>
        </div>
    @else
        <form wire:submit.prevent="submit" class="space-y-4">
            <h2 class="text-xl font-bold text-slate-900 dark:text-white mb-4">Envíanos un mensaje</h2>

            @if($selectedApp)
                <div class="p-3 bg-primary-50 dark:bg-primary-900/20 border border-primary-100 dark:border-primary-800 rounded-xl flex items-center gap-3 mb-5">
                    <div class="h-10 w-10 rounded-lg bg-primary-100 dark:bg-primary-800 flex items-center justify-center text-primary-600 dark:text-primary-400">
                    ...
                    <div>
                        <p class="text-[11px] font-semibold uppercase tracking-wider text-primary-600 dark:text-primary-400">Solicitando Demo para:</p>
                        <p class="text-base font-bold text-slate-900 dark:text-white">{{ $selectedApp->name }}</p>
                    </div>
                </div>
            @endif
            ...
            <div>
                ...
                @error('company') <p class="mt-1.5 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
            </div>
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div>
                    ...
                    @error('whatsapp') <p class="mt-1.5 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                </div>
            </div>
            ...
            <div>
                ...
                <div class="mt-1">
                    <textarea id="message" wire:model.blur="message" rows="3" class="block w-full rounded-xl shadow-sm bg-white dark:bg-slate-700 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-400 transition-colors" placeholder="¿Cómo podemos ayudarte?"></textarea>
                </div>
